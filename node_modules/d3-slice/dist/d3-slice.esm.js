import { scaleSequential, interpolateViridis, easeSin, keys, extent, pie, arc, interpolate } from 'd3';
...
function slice(container) {
  ...
  slice.pie = function (_) {
    return arguments.length ? (pie$$1 = _, slice) : pie$$1;
  };

  slice.arc = function (_) {
    return arguments.length ? (arc$$1 = _, slice) : arc$$1;
  };
  ...
  function slice() {
    ...
    pie$$1 = pie().value(function (d, i) {
      return valueExtractor(d, i);
    });
    arc$$1 = arc().outerRadius(outerRadius).innerRadius(innerRadius);
    labelArc = arc().outerRadius(labelRadius).innerRadius(labelRadius);
    ...
    slicePathData = pie$$1.padAngle(slicePadding)(dataKeys);
    slices.data(slicePathData).transition().duration(function (d, i) {
      return transitionDuration(d.data, d.index);
    }).ease(easeSin).attr('d', arc$$1.cornerRadius(cornerRadius)).attr('fill', function (d, i) {
    ...
    labels.transition().duration(function (d, i) {
      return transitionDuration(d.data, d.index);
    }).ease(easeSin).attrTween('transform', function (d) {
    ...
    labelLines.transition().duration(function (d, i) {
      return transitionDuration(d.data, d.index);
    }).ease(easeSin).attrTween('points', function (d) {
      this._current = this._current || d;
      var interpolate$$1 = interpolate(this._current, d);
      this._current = interpolate$$1(0);
      return function (t) {
        ...
        return [arc$$1.centroid(d2), labelArc.centroid(d2), pos];
      };
