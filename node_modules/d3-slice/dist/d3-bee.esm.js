import { scaleLinear, scaleSequential, interpolateViridis, easeSin, keys, extent, axisBottom, forceSimulation, forceX, forceY, forceCollide, voronoi } from 'd3';
...
function bee(container) {
  ...
  bee.forceCollide = function (_) {
    return arguments.length ? (forceCollide$$1 = _, bee) : forceCollide$$1;
  };
  ...
  function bee() {
    ...
    scaleR.domain(extent(dataRadii));
    ...
    simulation = forceSimulation(values).force('x', forceX(function (d, i) {
      return scaleX(valueExtractor(d.key, i));
    }).strength(1)).force('y', forceY(spaceY / 2)).force('collide', forceCollide(forceCollide$$1)).stop();
    ...
    let voroni = voronoi().extent([[0 - spaceRelax, 0 - spaceRelax], [spaceX + spaceRelax, spaceY + spaceRelax]]).x(d => d.x).y(d => d.y).polygons(values);
